...
export default function Ingredients() {
  ...
  const load = () => { client.get('/ingredients/').then(res => setItems(res.data)); };
  useEffect(load, []);
  ...
  return (
    <div>
      ...
      <div className="bg-charcoal-900 border border-charcoal-700 rounded-2xl overflow-hidden">
        ...
        {items.length === 0 && (
          <div className="text-center py-16 text-cream/40">No ingredients yet</div>
        )}
      </div>

      <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Ingredient">
